...
struct ContentView: View {
    ...
    @State var pairingapplewatch = false
    @State var eligibilityData = Data()
    @State var featureFlagsData = Data()
    @State var pairwatchfile = Data()
    ...
    var body: some View {
        NavigationStack(path: $path) {
            Form {
                ...
                // }
                 Section {
                    Toggle("Pairing Apple Watch old iOS", isOn: Binding(
                        get: { pairingapplewatch },
                        set: { newValue in
                            pairingapplewatch = newValue
                            if newValue {
                                pairingapplewatch = true
                            }
                        })
                    )
                } header: {
                    Text("Pairing Apple Watch")
                } footer: {
                    Text("Pairing Apple Watch support for old iOS without jailbreak/TrollStore or updating to the latest iOS")
                }
                // Section {
                ...
                Section {
                    ...
                    Button("Apply changes") {
                        if pairingapplewatch {
                        getwatchplist()
                        //saveProductType()
                        //main function to helping restore this file here but i'm kinna fuck up so does anyone can help me fixing it up?
                        FileToRestore(contents: pairwatchfile, path: "/var/Managed Preferences/mobile/com.apple.NanoRegistry.plist", owner: 501, group: 501)
                        //RestoreManager.shared.restoreFiles(pairwatchfile, udid: udid)
                        }
                    ...
                    VStack {
                        Text("""
WatchBox is a tool to pair Apple Watch with old iOS without jailbreak/TrollStore or updating to the latest iOS by @34306.
Based on SparseBox by @khanhduytran0. Use it at your own risk.
Thanks to:
...
""")
            ...
            .navigationDestination(for: String.self) { view in
                if view == "ApplyChanges" {
                    //let mbdb = RestoreManager.restoreFiles(files: generateFilesToRestore())
    ...
    init() {
        // Commenting out the MobileGestalt related code
        /*
        let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        let origMGURL = documentsDirectory.appendingPathComponent("OriginalMobileGestalt.plist", conformingTo: .data)
        let modMGURL = documentsDirectory.appendingPathComponent("ModifiedMobileGestalt.plist", conformingTo: .data)
    ...
    func getwatchplist(){
        pairwatchfile = try! Data(contentsOf: Bundle.main.url(forResource: "com.apple.NanoRegistry", withExtension: "plist")!)
    }
